...
class Logusu extends Controller {
						...
						success: function(r,s,x){
							try{
								...
								if (json.status == "A"){
									...
									'.$this->datasis->jwinopen(site_url('formatos/ver/LOGUSU').'/\'+res.id+\'/id\'').';
									return true;
								} else {
	...
	*/
	...
	function dataedit(){
		...
		$edit = new DataEdit($this->tits, 'logusu');
		...
		$edit->comenta = new textareaField('Comenta','comenta');
		$edit->comenta->rule='';
		$edit->comenta->cols = 70;
		$edit->comenta->rows = 4;
	...
	function ultimos(){
		...
		$mSQL = "SELECT fecha, hora, modulo, comenta FROM logusu WHERE usuario=".$usuario." AND modulo NOT IN ('MENU','PRINCIPAL') ORDER BY id DESC LIMIT 10";
		$query = $this->db->query($mSQL);
		if ( $query->num_rows() > 0 ) {
			foreach( $query->result() as $row ) {
				$mensaje = "<div class='msgln'>".substr($row->fecha,8,2)."-".substr($row->hora,0,5)." <b>".strtoupper($row->modulo)."</b>: ".$row->comenta."<br></div>\n".$mensaje;
			}
		}
		echo $mensaje;
	}
